...
async fn exec_replace_conflict(dsn: &str, batch_ids: &[u32], iterations: u32) -> Result<bool> {
    ...
    let mut sub_query = "select * from test_order3 where ".to_string();
...
async fn exec_replace(dsn: &str, batch_id: u32, iterations: u32) -> Result<bool> {
    ...
    let truncate_sql = "truncate table random_source_store3";
    ...
    let insert_sql = format!(
        "insert into random_source_store3 (select
                    id,
                    {batch_id} as id1,
                    ...
                    insert_time3,
                    i
              from random_source3 limit 1000)"
    );
    ...
    // insert data into test_order3 firstly.
    let insert_data_sql = format!("insert into test_order3 select * from random_source_store3");
    ...
    let sql = format!(
        "
         merge into test_order3 as t 
          using (
            select * from random_source_store3
          ) as s
...
async fn exec_table_maintenance(dsn: &str, batch_id: i32) -> Result<()> {
    ...
    let conn = new_connection(dsn)?;
    let sqls = vec![
        //"select * from test_order3 ignore_result",
        "optimize table test_order3 compact segment",
        "optimize table test_order3 compact",
        "optimize table test_order3 purge",
        "alter table test_order3 recluster",
    ];
    for sql in sqls {
        match conn.exec(sql).await {
            Ok(_) => {
...
async fn verify(dsn: &str, success_replace_stmts: u32, iterations: u32) -> Result<()> {
    ...
    // - check the table data match the number of successfully executed replace into statements
    {
        ...
        let mut rows = conn.query_iter("select count() from test_order3").await?;
        ...
        let mut rows = conn
            .query_iter(
                "
            select count() from
                (select count() a, id1 from test_order3  group by id1)
                where a != 1000",
            )
            .await?;
        let r = rows.next().await.unwrap().unwrap();
    ...
    {
        let mut rows = conn
            .query_iter("select count() from test_order3 where id2 != id1 * 7")
            .await?;
        let r = rows.next().await.unwrap().unwrap();
    ...
    {
        let rows = conn
            .query_iter("select * from test_order3 ignore_result")
            .await;
    ...
    let mut rows = conn
        .query_iter("select * from clustering_information('default', 'test_order3')")
        .await?;
    while let Some(r) = rows.next().await {
        let (
            cluster_key,
