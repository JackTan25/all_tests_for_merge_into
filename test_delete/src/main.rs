...
async fn exec_replace_conflict(dsn: &str, batch_ids: &[u32]) -> Result<bool> {
    ...
    let mut sub_query = "select * from test_order4 where ".to_string();
...
async fn exec_replace(dsn: &str, batch_id: u32) -> Result<bool> {
    ...
    let truncate_sql = "truncate table random_source_store4";
    ...
    let insert_sql = format!(
        "insert into random_source_store4 (select
                    id,
                    {batch_id} as id1,
                    ...
                    insert_time3,
                    i
              from random_source4 limit 1000)"
    );
    ...
    let sql = format!(
        "
         merge into test_order4 as t 
          using (
              select * from random_source_store4
          ) as s
...
//     let conn = new_connection(dsn)?;
//     let sqls = vec![
//         //"select * from test_order4 ignore_result",
//         "optimize table test_order4 compact segment",
//         "optimize table test_order4 compact",
//         "optimize table test_order4 purge",
//         "alter table test_order4 recluster",
//     ];
//     for sql in sqls {
//         match conn.exec(sql).await {
//             Ok(_) => {
...
async fn verify(dsn: &str, success_replace_stmts: u32) -> Result<()> {
    ...
    // - check the table data match the number of successfully executed replace into statements
    {
        ...
        let mut rows = conn.query_iter("select count() from test_order4").await?;
        ...
        // let mut rows = conn
        //     .query_iter("select count(distinct(id2)) from test_order4")
        //     .await?;
        // let r = rows.next().await.unwrap().unwrap();
        // let count: (u32,) = r.try_into()?;
    ...
    // {
    //     let mut rows = conn
    //         .query_iter("select count() from test_order4 where id2 != id1 * 7")
    //         .await?;
    //     let r = rows.next().await.unwrap().unwrap();
    ...
    {
        let rows = conn
            .query_iter("select * from test_order4 ignore_result")
            .await;
    ...
    let mut rows = conn
        .query_iter("select * from clustering_information('default', 'test_order4')")
        .await?;
    while let Some(r) = rows.next().await {
        let (
            cluster_key,
